...
// ─── Боты чата ────────────────────────────────────────────────────────────
const BOTS = [
  { name: 'Наташа',   avatar: 'Н', color: '#FF1FB3' },
  { name: 'Дима',     avatar: 'Д', color: '#7B2FF7' },
  { name: 'Алина',    avatar: 'А', color: '#A020F0' },
  { name: 'Серёга',   avatar: 'С', color: '#6366f1' },
  { name: 'Оля',      avatar: 'О', color: '#ec4899' },
  { name: 'Макс',     avatar: 'М', color: '#8b5cf6' },
  { name: 'Лена',     avatar: 'Л', color: '#f472b6' },
  { name: 'Антон',    avatar: 'А', color: '#818cf8' },
  { name: 'Кристина', avatar: 'К', color: '#c084fc' },
  { name: 'Влад',     avatar: 'В', color: '#a78bfa' },
  { name: 'Таня',     avatar: 'Т', color: '#f9a8d4' },
  { name: 'Паша',     avatar: 'П', color: '#7c3aed' },
  { name: 'Женя',     avatar: 'Ж', color: '#db2777' },
  { name: 'Коля',     avatar: 'К', color: '#4f46e5' },
  { name: 'Вика',     avatar: 'В', color: '#be185d' },
  { name: 'Рома',     avatar: 'Р', color: '#5b21b6' },
];
...
const BOT_REPLIES_TO_USER: Array<{ bot: number; text: string }> = [
  { bot: 0,  text: 'привет! крути, не пожалеешь 😊' },
  { bot: 1,  text: 'здарова! тут всё честно, я уже вывел сам' },
  { bot: 4,  text: 'добро пожаловать! на третьей попытке выйдет 🍀' },
  { bot: 2,  text: 'пиши менеджеру сразу после выигрыша, не тяни' },
  { bot: 3,  text: 'у меня тоже сначала сомнения были, но потом убедился' },
  { bot: 5,  text: 'крути скорее, акция не вечная!' },
  { bot: 0,  text: 'Екатерина всё объяснит, она быстро отвечает 💜' },
  { bot: 1,  text: 'и я так думал сначала. потом убедился сам 😄' },
  { bot: 4,  text: 'главное комиссию оплати сразу как выиграешь' },
  { bot: 2,  text: 'тут полчата уже вывело, реально работает 💯' },
  { bot: 6,  text: 'я тоже недавно зашла! давай вместе болеть за третью попытку)' },
  { bot: 7,  text: 'привет! третья попытка — твоя, удачи 🎯' },
  { bot: 8,  text: 'не бойся, тут всё официально и прозрачно' },
  { bot: 9,  text: 'я получил на прошлой неделе, всё чётко пришло' },
  { bot: 10, text: 'девчонки говорят что реально платят, я тоже проверила 😄' },
  { bot: 11, text: 'жди третьей! у меня точно так же было' },
  { bot: 3,  text: 'мне казалось странным платить комиссию, но потом понял — стандартно' },
  { bot: 5,  text: 'не тяни! акция ограничена по времени' },
  { bot: 0,  text: 'удачи! пиши как выиграешь, порадуемся вместе 🎉' },
  { bot: 1,  text: 'у меня знакомая тоже выиграла, говорит всё честно' },
];
...
const Index = () => {
  ...
  const sendChat = () => {
    ...
    const myMsg: ChatMsg = { bot: -1, text: chatInput.trim(), time: nowTime(), id: msgIdRef.current++ };
    setChat(c => [...c, myMsg]);
    setChatInput('');
    const count = rndInt(1, 3);
    for (let i = 0; i < count; i++) {
      const delay = 1800 + i * rndInt(1500, 2500);
      const reply = BOT_REPLIES_TO_USER[replyIdxRef.current % BOT_REPLIES_TO_USER.length];
      replyIdxRef.current++;
      setTimeout(() => {
        setTyping(reply.bot);
        setTimeout(() => {
